...
@author: jembrown
    ...
    Below is the beginning of a Node class definition and a simple example of
how to link nodes to form a tree. Use this as a springboard to start thinking
about:
...
# string is passed as an argument to the constructor and it automatically
# creates all the nodes and links them together. Start thinking about how to
# do that. Let's go ahead and define a Tree object that houses all these nodes
# and organizes methods associated with them.
import ctmc
...
class Tree:
    ...
    def __init__(self):
        ...
        # Now, let's add branch lengths to our Node objects (remember, these
        # fields can be added arbitrarily in Python). In the future, we should
        # probably include branch lengths in the Node constructor.
        self.spA.brl = 0.1
        self.spB.brl = 0.1
        self.spC.brl = 0.2
        self.ancAB.brl = 0.1
    ...
    # Write a recursive function that takes the root node as its only
    # argument and prints out all the names of the terminal nodes in the
    # tree. Due next Tues (3/17).
    def printNames(self, node):
        ...
        tip_nodes = []
        # checks to see if the node given is not a tip node
        if len(node.children) > 0:
            # for every child of that internal node
            for child in node.children:
                # checks if the child is not a tip node
                if len(child.children) > 0:
                    # use the function in each child of the argument node
                    tip_nodes.append(self.printNames(child))
                # appends the name of the child node if it is a tip node
                else:
                    tip_nodes.append(child.name)
        # appends the name of the argument node if it is a tip node
        else:
            tip_nodes.append(node.name)
        return tip_nodes


    # Write a recursive function to calculate the total tree length (the
    # sum of all the branch lengths). Again, the root node of a tree should
    # be the only argument the first time this function is called.
    # Due next Tues (3/17).
    ...
    # Write a recursive function that takes the root node as one of its
    # arguments and prints out a parenthetical (Newick) tree string.
    # Due next Tues (3/17).
    ...
    # Now, let's write a recursive function to simulate sequence evolution
    # along a tree. This amounts to simply simulating evolution along each
    # branch from the root towards the tips. We'll need to use our ctmc
    # class for setting the conditions of our simulation, which is why we
    # imported it above our tree class definition. In this case, we've
    # stored the definition of our ctmc class in a separate file (ctmc.py)
    # to keep our tree code compact. Now, let's add a ctmc object to each
    # internal node in our tree (except the root). Again, it would be best
    # to add the ctmcs as part of the Node constructor, if we know that
    # we'll be simulating data. Try to get this simulator and associated
    # functions working by next Thurs. (3/19).
